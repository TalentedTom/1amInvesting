...
| | SIVE (90) | FTC (71) |
|---|---|---|
| Monopoly | CW lasers for CPO | E-band GaN SSPAs for Starlink |
| Customer validation | NVIDIA $4B to customers | SpaceX 15% warrants + £47M order |
| Market cap | ~$1B | ~£620M |
| TAM expansion | Datacenter → LiDAR → defense | Ground stations → consumer terminals → defense |
| Supercycle | DB1 Photonics | DB3 LEO Space + DB8 Defense |
| Revenue | ~SEK 300M+ | ~£30-40M |
| Sovereignty | Swedish (European chokepoint) | UK (European chokepoint) |
| V-band / SATCOM | SATCOM BFICs (2027-2030) | V-band consumer (2031-2033, discounted) |
...
REVENUE TRAJECTORY:
  FY2025: ~£25-30M
  FY2026: ~£35-45M (ramp)
  FY2027: ~£60-90M (£47.3M GaN order shipping + defense)
  FY2028: ~£100-150M (GaN at scale + defense + V-band early)
  FY2029: ~£150-200M (E-band maturity + defense ramp)
  FY2031-33: £300-1.2B (if V-band consumer terminal succeeds)

EARNINGS MODEL (with years):
  £90M (FY2027), 20% OP = £18M × 35x = £630M → GBp ~290
  £150M (FY2028-29), 22% OP = £33M × 40x = £1.32B → GBp ~600
  £250M (FY2029-30), 25% OP = £62.5M × 40x = £2.5B → GBp ~1,165
  V-band bull (FY2032-33): £750M, 30% OP = £225M × 30x = £6.75B → GBp ~3,150

RULE 16 BLENDED CEILING:
  Near-term E-band + defense (2028-29): GBp 500-800
  Long-term V-band incremental (2031-33, ~6yr out): GBp 1,000-2,200
  Discounted at 20%/yr (0.8^6 = 0.262): GBp 262-576
  
  BLENDED CEILING: GBp 750-1,400
  (Re-run when V-band prototype ships — discount drops to 0.8^3 = 0.512)
```

**Entry:** GBp 296 vs blended midpoint GBp 1,075 = 2.75x → Entry 60
...
## Score Change Log
- Apr 23, 2026: 72 WL (initial deep dive)
