...
- Consumer wearable TAM beyond Apple: Samsung + Huawei + Fitbit = $240-300M (not modeled).
- NASDAQ listing: reprices MC to US institutional liquidity.
...
|------|---------|----------|-------|--------|-----------|
| 2027 | $260M | $104M | $3.12B | 355.08M | **SEK 93** (93.1) |
| 2028 | $751M | $300M | $9.01B | 355.08M | **SEK 269** (269.0) |
| 2029 | $1,230M | $492M | $14.76B | 355.08M | **SEK 441** (440.6) |
| 2030 | $1,665M | $666M | $19.98B | 355.08M | **SEK 596** (596.4) |

(USD/SEK 10.6)

**When to apply the Nasdaq dilution:** on the day shares are actually issued. At that point, affected FY columns recompute at ~409M shares. The Nasdaq listing catalyst (institutional access, US liquidity, potential multiple re-rate above the 20x standard) arrives WITH the dilution — both modeled simultaneously when the event occurs.
...
**Symmetric-hire epistemics:** this artifact scores Wuischpard→NVIDIA as a bullish Stage-3 tell; Seyedi→ams OSRAM is the mirror-image tell and receives the same weight class — an ex-insider betting the scale-up stage isn't ELS's by right. Mechanism probability: **25-35%** that VCSEL-based CPO materially contests the scale-up/on-package tier by FY29.

**Domain mapping (why Base −1, not more):** the contested tier is Stage 3 optical I/O — explicitly OUT of the base model (free option). Base-case vectors (switch-CPO ELS, Jabil 1.6T single-mode pluggable, defense/SATCOM, LiDAR) are VCSEL-immune on physics through the modeled spine: no WDM density in multimode, 200G/lane VCSEL at lab edge, reach collapse at speed, ecosystem sunk into SiPh+ELS (TSMC COUPE, GFS reference designs, shipping Broadcom/NVIDIA platforms). ams OSRAM starts from zero datacom interconnect business — design-win → HVM 2028-29 earliest. Most-likely branch: cheap VCSEL optical I/O displaces **copper** first (DAC/AEC tier) — TAM-additive for photonics, CRDO-adjacent, not ELS-destructive.

**Pre-registered tells:**
1. ams OSRAM names foundry/OSAT partners or customer co-development for the BU → re-review: Base −1 further OR Stage-3 option haircut, same turn.
2. NVIDIA/Broadcom roadmap shows VCSEL-based scale-up on the Feynman generation → Stage-3 option haircut + Rule 20 photonics-cluster pass (SIVE, PCL, TSEM, FOCI notes).
3. COUNTER: no 200G/lane VCSEL demo at OFC 2027, or ELS attach rates hold in Rubin-gen CPO SKUs → class de-escalates, Risks review −3→−2.
4. COUNTER: ams OSRAM BU explicitly targets copper-replacement short-reach → threat reclassifies CRDO-adjacent (TAM-additive), SIVE class closes.

**Cross-book:** Win Semi = anti-fragile node (world's leading GaAs foundry captures VCSEL volume as naturally as InP — both branches feed it). COHR/LITE/AIXA hedged (VCSEL franchises + epi tools span both). TSEM + PCL are the other ELS/SiPh-pure exposures — class-note on next touch. ams OSRAM (AMS.SW) screen: flagged-parked.
