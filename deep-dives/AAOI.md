# AAOI — Applied Optoelectronics | Deep Dive
**Date:** April 24, 2026
**Framework Version:** v3.6.2
**Score:** 64 | Base 68% (41/60) | Entry 58 (23/40) | **Owned ✓**
**Price at Analysis:** $162.17
**Ceiling:** $300-$600
**Upside Ratio:** 1.9x-3.7x
**Market Cap:** ~$10B
**FY2025 Revenue:** $455.7M (+83% YoY)
**FY2026 Guidance:** >$1B revenue, >$120M non-GAAP OP
**Earnings:** ~May 7, 2026 🔥

---

## Supercycle: DB1 Photonics (Transceiver Assembly — Layer 1)

## Core Thesis: "Buy ALL Production"

A hyperscaler told AAOI management they would buy ALL production. The inference from Rule 7: if ONE hyperscaler is capacity-constrained on transceivers, the others are too. CFO: "Revenue limited by production capacity and supply chain, not market demand, which we believe is much larger."

## The Revenue Explosion

| Period | Revenue | Key Driver |
|--------|---------|-----------|
| FY2024 | $249.4M | Pre-ramp |
| FY2025 | $455.7M (+83%) | 400G volume + CATV |
| Q4 2025 | $134.3M (+34%) | 800G begins |
| Q1 2026E | $150-165M | 800G ramping |
| **FY2026E** | **>$1B (guided)** | **800G + 1.6T** |
| **Mid-2027** | **$378M/month** | **$4.5B annualized!** |

## Recent Orders

- $124M total 800G orders from lead hyperscaler since mid-March
- $200M+ first 1.6T volume order (March 9, 2026)
- 1.6T LPO shipments began March 2026
- Second hyperscaler customer: shipped first 10,000 units of 800G
- 25dBm ultra-high-power ELSFP showcased at OFC 2026

## Manufacturing Expansion

- Sugar Land TX: 210K sqft new facility, operational Q4 2026
- Pearland TX: two additional industrial properties acquired April 2026
- Total Houston footprint: approaching 900K sqft
- Target: 500K units/month 800G+1.6T by end-2026
- Mid-2027: tripling laser manufacturing in Texas
- "Made in USA" status valuable for sovereign AI + government contracts

## Why AAOI Is NOT a Monopoly (Framework Honest)

AAOI is Layer 1 (transceiver assembly), not Layer 0 (components). The framework distinguishes:

| Layer | Company | Position | Moat |
|-------|---------|----------|------|
| Layer 0 (laser) | SIVE, LITE, COHR | Monopoly/near-monopoly | Physics-required |
| Layer 0 (test) | AEHR | Sole-source WLBI | No competitor exists |
| **Layer 1 (transceiver)** | **AAOI, Innolight, CIG, Eoptolink** | **Competitive** | **Capacity + speed** |

AAOI competes against Innolight ($1.87B Q4 revenue — 14x AAOI's DC revenue), Eoptolink (>$1B), CIG Shanghai, Coherent, and Lumentum. The moat is execution speed and customer relationships, not physics.

**"Competitive companies go up in supercycles. Monopolies go up MORE."** (Framework Axiom 4)

## Scoring Breakdown

| Category | Score | Evidence |
|----------|-------|---------|
| Core Thesis | 13/20 | "Buy ALL production." $1B guidance credible. 1.6T first mover. BUT: competitive market (-3), customer concentration 91% from 3 (-2), LightCounting oversupply warning (-1), not vertically integrated (-1). |
| Cycle Stage | 8/10 | Revenue doubling. $324M orders. 1.6T shipping. Stage 3. |
| TAM | 7/10 | Transceivers $23.8B→$80-120B. Phase 1 only. |
| Catalysts | 5/5 | Q1 May 7 🔥. 800G ramp. 1.6T Q3 volume. Sugar Land Q4. Profitability Q2. |
| Macro | 8/10 | 120-150 GW. 800G + 1.6T. Every GPU needs transceivers. |
| Alpha | 3/5 | "Buy ALL production" Rule 7 signal. ~12 analysts. Thesis known. |
| Risks | -3 | Concentration 91% (-1). LightCounting oversupply (-1). Not vertically integrated (-1). |
| **Base** | **41/60 (68%)** | |

## Ceiling — Forward TAM Math

```
FY2025: $456M → FY2026: $1B+ → FY2027: $2-3B → FY2028: $3-5B

Mid-2027 projection breakdown (management):
  100G/400G: $90M/month
  800G: $217M/month
  1.6T: $71M/month
  Total: $378M/month = $4.5B annualized

EARNINGS:
  $1B, 15% OP = $150M × 25x = $3.75B → ~$60
  $2B, 20% OP = $400M × 28x = $11.2B → ~$180
  $3B, 22% OP = $660M × 30x = $19.8B → ~$320
  $4.5B (mid-2027 run rate), 23% OP = $1.04B × 30x = $31B → ~$500

CEILING: $300-$600
```

**Entry:** $162.17 vs midpoint $450 = 2.78x → Entry 58

## Total: (68 × 0.6) + (58 × 0.4) = 41 + 23 = 64

## Score Change: 68 → 64

The stock appreciated from ~$90 (when originally scored 68) to $162, compressing the Entry. The base stays at 68% — fresh data ($200M+ 1.6T order, $378M/month projection) RAISES the ceiling, but the price moved faster. Dynamic scoring working correctly.

## AAOI's Role in the Supply Chain

AAOI is the CUSTOMER of our monopoly positions:
- AAOI buys EML lasers from LITE
- AAOI buys laser components that originated on InP from AXTI
- Those lasers were tested by AEHR's WLBI
- Those wafers were grown by AIXA.DE's MOCVD tools

When AAOI ships $1B in transceivers, every unit contains components from our Layer 0 positions. AAOI's revenue growth IS demand for SIVE/LITE/COHR/AXTI/AEHR.

## Key Risks

- **Customer concentration:** 3 customers = 91% of revenue. Microsoft reportedly ~29%. Catastrophic if one slows.
- **LightCounting warning:** "Production capacity catching up with demand... may lead to sharper price declines by end of 2026." Non-vertically-integrated assemblers (AAOI) most exposed.
- **Not vertically integrated:** Buys lasers from LITE/COHR. If laser supply tightens, AAOI can't manufacture its own.
- **Still unprofitable:** FY2025 EPS -$0.26. Approaching breakeven but not there yet.
- **$500M ATM dilution:** Equity program dilutes shareholders to fund expansion.

## Verdict

64 — just below WL threshold. The revenue explosion is real ($456M→$1B guided). "Buy ALL production" is the ultimate demand signal. But AAOI is Layer 1 competitive, not Layer 0 monopoly. Customer concentration (91%) and LightCounting oversupply warning create material risk. The framework owns it at Tier 3 because it's a direct demand signal for our Layer 0 positions, and trusted investors are bullish.
